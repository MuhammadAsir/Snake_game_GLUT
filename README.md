...
| Early Game | Mid Game |
|:---:|:---:|
| ![Snake game at score 0, length 5, level 1](screenshot/snake%20game%20ss1.PNG) | ![Snake game at score 80, length 13, level 2, with score pop-up](screenshot/ss2.PNG) |
...
This project requires a C++ environment configured with the OpenGL/GLUT library. The instructions below outline how to set up the project using the Code::Blocks IDE.

### 1. Prerequisites

Before proceeding, ensure that Code::Blocks and the MinGW compiler are installed. You will also need the freeGLUT development files (headers, libraries, and DLLs) configured on your system.

### 2. Project Initialization

1. Launch Code::Blocks and navigate to **File → New → Project**.
2. From the list of templates, select **GLUT Project**.
3. Define the Project Title (e.g., `SnakeGame2D`) and choose your working directory.
4. When prompted for the GLUT Location, browse to the directory where your library files are stored (typically the MinGW folder or a specific freeglut directory).
   > **Tip:** Ensure the directory contains the `include` and `lib` subfolders.

### 3. Compiler Verification

If the project fails to compile initially, verify your toolchain settings:

- Go to **Settings → Compiler**.
- Select the **Global Compiler Settings** tab.
- Click on **Toolchain Executables**.
- Ensure the **Compiler's installation directory** accurately reflects the path to your MinGW folder.

### 4. Code Integration

By default, Code::Blocks generates a template file. To run this game:

1. In the Management side panel, expand the **Sources** folder and open `main.cpp`.
2. Delete all existing boilerplate code within the editor.
3. Copy the source code from this repository's `main.cpp` and paste it into the editor.
4. Save the file and press **F9** (or go to **Build → Build and Run**).
...
| Name | ID |
|---|---|
| Muhammad Asir Hossain Chowdhury | 233021512 |
| Sumaita Islam | 233004012 |
